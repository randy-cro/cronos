...
def test_staking_cache_unbonding_validator(cronos_staking_cache):
    """
    Test unbonding a validator by removing all self-delegation.

    This test verifies that when a validator unbonds all their self-delegation,
    the validator transitions to unbonding state correctly across all nodes
    with different cache configurations.
    """
    cronos = cronos_staking_cache

    # We'll use node 4 (cache-size=3) validator for this test
    test_node_idx = 4
    ...
    print("\n=== Testing validator unbonding ===")
    print(f"Validator address: {val_addr}")
    print(f"Validator account: {val_account}")
    ...
    for node_idx in range(5):
        ...
        # Find our test validator
        test_val = None
        for v in validators:
            if v["operator_address"] == val_addr:
                test_val = v
                break
    ...
    print(f"\nValidator's self-delegations: {delegations}")

    # Query the specific delegation to this validator
    delegation_info = cli.get_delegations(val_account)
    if delegation_info and "delegation_responses" in delegation_info:
        for del_resp in delegation_info["delegation_responses"]:
            if del_resp["delegation"]["validator_address"] == val_addr:
                ...
                print("\n=== Phase 2: Unbonding validator self-delegation ===")
                print(f"Unbonding {unbond_amount} from validator")
                ...
                print(
                    "\n=== Phase 3: Validator status after unbonding "
                    "(should be UNBONDING) ==="
                )
                unbonding_statuses = []
                for node_idx in range(5):
                ...
                assert len(set(unbonding_statuses)) == 1, (
                    f"Validator has different statuses across nodes: "
                    f"{unbonding_statuses}"
                )
                assert (
                ...
                print(
                    "\n=== Phase 5: Checking validator count "
                    "after unbonding period ==="
                )
                initial_validator_count = 5  # We started with 5 validators
                validator_counts = []
                ...
                # Assert validator count reduced by 1
                expected_count = initial_validator_count - 1
                assert validator_counts[0] == expected_count, (
                    f"Expected {expected_count} validators but got "
                    f"{validator_counts[0]}"
                )
                msg = (
                    f"✓ All nodes consistent: {validator_counts[0]} "
                    f"validators (reduced by 1)"
                )
                print(msg)
...
def test_staking_cache_consistency(cronos_staking_cache):
    ...
    # Use validator 4 for unbonding test
    test_node_idx = 4
    val_cli = cronos.cosmos_cli(test_node_idx)
    val_addr = val_cli.address("validator", bech="val")

    print(f"Unbonding validator {test_node_idx}: {val_addr}")
    ...
    # Unbond 1 token from validator's self-delegation
    unbond_val_amount = "1000000000000000000stake"
    print(f"Unbonding {unbond_val_amount} from validator self-delegation")
    rsp = val_cli.unbond_amount(val_addr, unbond_val_amount, "validator")
    assert rsp["code"] == 0, f"Validator unbonding failed: {rsp.get('raw_log', rsp)}"
    wait_for_new_blocks(cli, 1)

    # Verify validator is in UNBONDING status across all nodes
    ...
    # Check validator count reduced by 1
    ...
    msg = f"✓ All nodes consistent: {validator_counts[0]} validators (reduced by 1)"
    print(msg)
